...
from .my_math import spatial_tensor_multiply
...
def calculate_eddy_viscosities(base_tensors: Dict[str, np.ndarray]) -> np.ndarray:
    ...
    nut = np.trace(
        spatial_tensor_multiply(a_mat, s_mat.transpose(1, 0, 2, 3))
    ) / (2 * np.trace(spatial_tensor_multiply(s_mat, s_mat.transpose(1, 0, 2, 3))))
...
def qcr_alignment(base_tensors: Dict[str, np.ndarray], version: int) -> np.ndarray:
    ...
    if version == 2000:
        ...
        rhs = 2 * s_mat + 2 * c_qcr_1 * (
            spatial_tensor_multiply(s_mat, o_mat)
            - spatial_tensor_multiply(o_mat, s_mat)
        )
    elif version == 2013:
        lhs = -r_mat
        rhs = (
            ...
            * (
            ...
            - c_qcr_2
            * np.sqrt(
                2 * np.trace(spatial_tensor_multiply(s_mat, s_mat), axis1=0, axis2=1)
            )[None, None, :, :]
            * np.eye(3)[:, :, None, None]
        )
...
def gatski_and_speziale_alignment(
    ...
    tke_production = -np.trace(
        spatial_tensor_multiply(r_mat, dv_mat.transpose(1, 0, 2, 3)),
        axis1=0,
        axis2=1,
    )
    tke = 0.5 * np.trace(r_mat, axis1=0, axis2=1)
    ...
    p_1_b = np.trace(
        spatial_tensor_multiply(b_mat, b_mat.transpose(1, 0, 2, 3)),
        axis1=0,
        axis2=1,
    )  # n x n
    ...
    # eta, n x n
    eta1 = np.trace(spatial_tensor_multiply(s_star, s_star), axis1=0, axis2=1)
    eta2 = np.trace(spatial_tensor_multiply(w_star, w_star), axis1=0, axis2=1)
    eta3 = np.trace(
        spatial_tensor_multiply(spatial_tensor_multiply(s_star, s_star), s_star),
        axis1=0,
        axis2=1,
    )
    eta4 = np.trace(
        spatial_tensor_multiply(s_star, spatial_tensor_multiply(w_star, w_star)),
        axis1=0,
        axis2=1,
    )
    eta5 = np.trace(
        spatial_tensor_multiply(
            spatial_tensor_multiply(s_star, s_star),
            spatial_tensor_multiply(w_star, w_star),
        ),
        axis1=0,
        axis2=1,
    )
...
def construct_pope_integrity_basis(
    ...
    t_2 = spatial_tensor_multiply(s_star, w_star) - spatial_tensor_multiply(
        w_star, s_star
    )
    ...
    t_3 = (
        spatial_tensor_multiply(s_star, s_star)
        - (1 / 3)
        * np.trace(spatial_tensor_multiply(s_star, s_star), axis1=0, axis2=1)[
            None, None, :, :
        ]
        * np.eye(3)[:, :, None, None]
    )
    ...
    t_4 = (
        spatial_tensor_multiply(w_star, w_star)
        - (1 / 3)
        * np.trace(spatial_tensor_multiply(w_star, w_star), axis1=0, axis2=1)[
            None, None, :, :
        ]
        * np.eye(3)[:, :, None, None]
    )

    t_5 = spatial_tensor_multiply(
        w_star, spatial_tensor_multiply(s_star, s_star)
    ) - spatial_tensor_multiply(spatial_tensor_multiply(s_star, s_star), w_star)

    t_6 = (
        spatial_tensor_multiply(spatial_tensor_multiply(w_star, w_star), s_star)
        + spatial_tensor_multiply(s_star, spatial_tensor_multiply(w_star, w_star))
        - (2 / 3)
        * np.trace(
            spatial_tensor_multiply(s_star, spatial_tensor_multiply(w_star, w_star)),
    ...
    t_7 = spatial_tensor_multiply(
        spatial_tensor_multiply(w_star, s_star), spatial_tensor_multiply(w_star, w_star)
    ) - spatial_tensor_multiply(
        spatial_tensor_multiply(w_star, w_star), spatial_tensor_multiply(s_star, w_star)
    )

    t_8 = spatial_tensor_multiply(
        spatial_tensor_multiply(s_star, w_star), spatial_tensor_multiply(s_star, s_star)
    ) - spatial_tensor_multiply(
        spatial_tensor_multiply(s_star, s_star), spatial_tensor_multiply(w_star, s_star)
    )
