...
def velocity_std_due_to_rotation(
    ...
    du = prefactor * np.pi * np.sqrt(profile["mean_velocity"][components[1]] ** 2) / 180
    dv = prefactor * np.pi * np.sqrt(profile["mean_velocity"][components[0]] ** 2) / 180
...
def stress_std_due_to_rotation(
    ...
    duu = (
        prefactor * np.pi * np.sqrt(profile["reynolds_stress"][components[3]] ** 2) / 90
    )
    dvv = (
        prefactor * np.pi * np.sqrt(profile["reynolds_stress"][components[3]] ** 2) / 90
    )
    ...
    duw = (
        prefactor
        * np.pi
        * np.sqrt(profile["reynolds_stress"][components[5]] ** 2)
        / 180
    )
    dvw = (
        prefactor
        * np.pi
        * np.sqrt(profile["reynolds_stress"][components[4]] ** 2)
        / 180
    )
...
def velocity_std_due_to_random(
    ...
    du = np.sqrt(profile["reynolds_stress"][components[0]] / n_eff)
    dv = np.sqrt(profile["reynolds_stress"][components[1]] / n_eff)
    dw = np.sqrt(profile["reynolds_stress"][components[2]] / n_eff)
    return du, dv, dw
...
def stress_std_due_to_random(
    ...
    duu = np.sqrt(2 * profile["reynolds_stress"][components[0]] ** 2 / n_eff)
    dvv = np.sqrt(2 * profile["reynolds_stress"][components[1]] ** 2 / n_eff)
    dww = np.sqrt(2 * profile["reynolds_stress"][components[2]] ** 2 / n_eff)

    Ruv = (
        profile["reynolds_stress"][components[3]]
        / np.sqrt(profile["reynolds_stress"][components[0]])
        / np.sqrt(profile["reynolds_stress"][components[1]])
    )
    Ruw = (
        profile["reynolds_stress"][components[4]]
        / np.sqrt(profile["reynolds_stress"][components[0]])
        / np.sqrt(profile["reynolds_stress"][components[2]])
    )
    Rvw = (
        profile["reynolds_stress"][components[5]]
        / np.sqrt(profile["reynolds_stress"][components[1]])
        / np.sqrt(profile["reynolds_stress"][components[2]])
    )
