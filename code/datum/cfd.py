"""This module defines functions for working with ANSYS Fluent CFD data and Tecplot
files."""
import os
import re
...
def normalize_variables_by_reference(
    ...
    :param calculate_reynolds_stress: A boolean indicating whether the Reynolds stress
        tensor should additionally be computed. If the value is true, the Reynolds
        stress tensor components will be computed.
    """
    # pylint: disable=too-many-locals
...
@log.log_process("Extract a profile from the Fluent solution", "sub")
...
) -> Tuple[Dict[str, Dict[str, np.ndarray]], tp.data.Dataset]:
