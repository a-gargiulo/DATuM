...
class PoseWindow:
    ...
    def __init__(self, master: tk.Toplevel, piv: Piv, geometry: Beverli, parameters_status: tk.BooleanVar):
        """
        Class constructor.
    ...
    def create_widgets(self):
        ...
        self.checkbox_diagonal = Checkbutton(
            self.options_section.get_content_frame(), category=1, text="Plane is diagonal"
        )
        self.mode_selector_label = Label(
            self.options_section.get_content_frame(), text="Select Calculation Mode:", category=1
        )
        ...
        self.mode_selector = tk.OptionMenu(
            self.options_section.get_content_frame(), self.mode_selector_var, *CALCULATION_MODES
        )
        self.submit_button = Button(self.main_frame, text="Submit Transformation File", command=self.submit_file)
        self.parameters_loader = FileLoader(
            self.options_section.get_content_frame(),
        ...
        self.calplate_loader.get_status_label_var().trace(
            "w",
            lambda *args: (
                self.create_local_pose_selector(self.mode_selector_var.get(), *args)
                if self.calplate_loader.get_status_label_var().get() == "File Loaded"
                and self.global_loader.get_status_label_var().get() == "File Loaded"
                else None
            ),
        )
        ...
        self.global_loader.get_status_label_var().trace(
            "w",
            lambda *args: (
                self.create_local_pose_selector(self.mode_selector_var.get(), *args)
                if self.calplate_loader.get_status_label_var().get() == "File Loaded"
        ...
        self.measurement_loader.get_status_label_var().trace(
            "w",
            lambda *args: (
                self.create_global_pose_calculator(self.mode_selector_var.get(), *args)
                if self.calplate_loader.get_status_label_var().get() == "File Loaded"
                and self.measurement_loader.get_status_label_var().get() == "File Loaded"
                else None
            ),
        )
        self.calplate_loader.get_status_label_var().trace(
        ...
        self.calplate_plot = Frame(self.local_section.get_content_frame(), category=2, bd=2, relief="solid")
        self.picker_frame = Frame(self.local_section.get_content_frame(), category=2)
        self.picker_button = Button(self.picker_frame, text="Pick Location", command=self.pick_location)
        self.picker_monitors = Frame(self.picker_frame, category=2)
        self.xpick_var = tk.StringVar()
        self.xpick_entry_label = Label(self.picker_monitors, text="x1 [mm]:", category=2)
        self.xpick_entry = Entry(self.picker_monitors, category=2, textvariable=self.xpick_var, state="readonly")
        self.ypick_entry_label = Label(self.picker_monitors, text="x2 [mm]:", category=2)
        self.ypick_var = tk.StringVar()
        self.ypick_entry = Entry(self.picker_monitors, category=2, textvariable=self.ypick_var, state="readonly")
        self.global_section = Section(self.main_frame, title="Global Pose", category=2)
        self.global_plot = Frame(self.global_section.get_content_frame(), category=2, bd=2, relief="solid")
        self.is_convex_option = Checkbutton(
            self.global_section.get_content_frame(), category=2, text="Use Convex Hill Curvature Correction"
        )
        self.use_measured_angle_option = Checkbutton(
            self.global_section.get_content_frame(), category=2, text="Use measured angle"
        )
        self.calculate_global_button = Button(
            self.global_section.get_content_frame(), text="Calculate Global", command=self.calculate_global
        )
    ...
    def layout_widgets_default(self):
        ...
        self.options_section.grid(row=0, column=0, padx=PAD_S, pady=PAD_S, sticky="nsew")
        self.options_section.get_content_frame().grid_columnconfigure(0, weight=1)
        self.options_section.get_content_frame().grid_columnconfigure(1, weight=1)
        self.checkbox_diagonal.grid(row=0, column=0, padx=PAD_S, pady=PAD_S, sticky="nsew")
        self.mode_selector_label.grid(row=1, column=0, padx=PAD_S, pady=PAD_S, sticky="w")
        self.mode_selector.grid(row=1, column=1, padx=PAD_S, pady=PAD_S, sticky="ew")
        self.parameters_loader.grid(row=2, column=0, columnspan=2, padx=PAD_S, pady=PAD_S, sticky="nsew")
    ...
    def layout_widgets_local(self):
        ...
        self.calplate_loader.grid(row=1, column=0, columnspan=2, padx=PAD_S, pady=PAD_S, sticky="nsew")
        self.global_loader.grid(row=2, column=0, columnspan=2, padx=PAD_S, pady=PAD_S, sticky="nsew")
    ...
    def layout_widgets_global(self):
        ...
        self.mode_selector_label.grid(row=0)
        self.calplate_loader.grid(row=1, column=0, columnspan=2, padx=PAD_S, pady=PAD_S, sticky="nsew")
        self.measurement_loader.grid(row=2, column=0, columnspan=2, padx=PAD_S, pady=PAD_S, sticky="nsew")
        self.submit_button.grid(row=2, column=0, padx=PAD_S, pady=PAD_S)
        self.scrollable_canvas.configure_frame()
    ...
    def plot_calplate(self, case: str):
        ...
        # TODO: a fixed value for skiprows may be too specific. Generalize!
        ...
        if case == CALCULATION_MODES[1]:
            ...
            transformation_parameters = apputils.read_json(transformation_parameters_path)
            if transformation_parameters is None:
                sys.exit(1)
            rotation_angle_deg = float(cast(dict, transformation_parameters["rotation"])["angle_deg"])
    ...
    def create_local_pose_selector(self, case: str, *args):
        ...
        self.local_section.grid(row=row, column=0, padx=PAD_S, pady=PAD_S, sticky="nsew")
        ...
        self.xpick_entry_label.grid(row=0, column=0, padx=PAD_S, pady=PAD_S, sticky="nsew")
        self.ypick_entry_label.grid(row=0, column=1, padx=PAD_S, pady=PAD_S, sticky="nsew")
    ...
    def create_global_pose_calculator(self, case: str, *args):
        ...
        self.is_convex_option.grid(row=0, column=0, padx=PAD_S, pady=PAD_S, sticky="nsew")
        self.use_measured_angle_option.grid(row=0, column=1, padx=PAD_S, pady=PAD_S, sticky="nsew")
        self.calculate_global_button.grid(row=0, column=2, padx=PAD_S, pady=PAD_S, sticky="nsew")
    ...
    def calculate_global(self):
        ...
        self.global_plot.grid(row=1, column=0, columnspan=3, padx=PAD_S, pady=PAD_S, sticky="nsew")
        opts = {
            "apply_convex_curvature_correction": bool(self.is_convex_option.get_var().get()),
            "use_measured_rotation_angle": bool(self.use_measured_angle_option.get_var().get()),
        }
    ...
    def submit_file(self):
        ...
        if case == CALCULATION_MODES[0]:
            ...
            if self.checkbox_diagonal and bool(self.checkbox_diagonal.get_var().get()):
                self.piv.pose.angle1 = float(cast(dict, transformation_parameters["rotation"])["angle_1_deg"])
                self.piv.pose.angle2 = float(cast(dict, transformation_parameters["rotation"])["angle_2_deg"])
            else:
                self.piv.pose.angle1 = float(cast(dict, transformation_parameters["rotation"])["angle_deg"])
            self.piv.pose.loc[0] = float(cast(dict, transformation_parameters["translation"])["x_1_loc_ref_mm"])
            self.piv.pose.loc[1] = float(cast(dict, transformation_parameters["translation"])["x_2_loc_ref_mm"])
            self.piv.pose.glob[0] = float(cast(dict, transformation_parameters["translation"])["x_1_glob_ref_m"])
            self.piv.pose.glob[1] = float(cast(dict, transformation_parameters["translation"])["x_2_glob_ref_m"])
            self.piv.pose.glob[2] = float(cast(dict, transformation_parameters["translation"])["x_3_glob_ref_m"])
